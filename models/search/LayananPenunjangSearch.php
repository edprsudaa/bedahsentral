...
/**
 * LayananPenunjangSearch represents the model behind the search form of `app\models\pendaftaran\Layanan`.
 */
class LayananPenunjangSearch extends Layanan
{
    public $pasien;
    public $pjp;
    /**
    ...
    public function search($params,$user)
    {
        ...
        if($user['akses_level']==HelperSpesial::LEVEL_DOKTER){
            $query->andWhere([
                TbPegawai::tableName() . '.pegawai_id'=> $user['pegawai_id']
            ]);
        }else{  
            $akses_pengguna=HelperSpesial::getListPenunjangAksesPegawai(false,$user);
        ...
        $this->jenis_layanan=self::PENUNJANG;
