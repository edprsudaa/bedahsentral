...
class ItemPemeriksaanPenunjang extends \yii\db\ActiveRecord
{
    ...
    public static function getListData($q=null){
        ...
        if(!empty($q)){
            ...
            select
                id ,deskripsi as text, rumpun,CONCAT('(',rumpun,') ',deskripsi) as text_rumpun
            FROM parents induk  where induk.id not in (select distinct parent from parents where parent is not null) AND (LOWER(rumpun) ilike '%".strtolower($q)."%' OR LOWER(deskripsi) ilike '%".strtolower($q)."%')
            ";
        }
