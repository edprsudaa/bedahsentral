...
class Icd9cm extends \yii\db\ActiveRecord
{
    ...
    public static function getListProsedur($q=null){
        ...
        FROM parents icd  where icd.id not in (select distinct parent from parents where parent is not null) AND (LOWER(kode) ilike '%".strtolower($q)."%' OR LOWER(deskripsi) ilike '%".strtolower($q)."%')
        ";
