...
use app\models\search\InformConcentTindakanDokterSearch;
...
use app\components\mdcp\MdcpInformConcentTindakanDokter;
...
/**
 * InformConcentTindakanDokterController implements the CRUD actions for InformConcentTindakanDokter model.
 */
class InformConcentTindakanDokterPasienController extends Controller
{
  /**
   * {@inheritdoc}
   */
  ...
  /**
   * Lists all InformConcentTindakanDokter models.
   * @return mixed
   */
  public function actionIndex($id)
  {
    ...
    if ($model) {
      return $this->redirect(Url::to(['/inform-concent-tindakan-dokter-pasien/update/', 'id' => $id, 'subid' => $model->incon_id]));
    } else {
      return $this->redirect(Url::to(['/inform-concent-tindakan-dokter-pasien/create/', 'id' => $timoperasi->to_ok_pl_id]));
    }
  }
  public function actionCreate($id)
  {
    ...
    $title = InformConcentTindakanDokter::title;
  ...
  public function actionUpdate($id, $subid)
  {
    ...
    $model->incon_tindakan_inform_consent = json_decode($model->incon_tindakan_inform_consent);
  ...
  {
    $transaction = InformConcentTindakanDokter::getDb()->beginTransaction();
    try {
      $s_flag = true;
      ...
      // if ($final || $batal) {
      //   if ($s_flag) {
      //     $data = \Yii::$app->controller->renderPartial('doc', ['incon_id' => $model->incon_id]);
      //     $mdcp_base = MdcpBase::getLayanan($model->timoperasi->to_ok_pl_id);
      //     $mdcp = MdcpInformConcentTindakanDokter::_set($model->incon_id, $mdcp_base, [], $data, $batal);
      ...
      if ($s_flag) {
        $transaction->commit();
        return MakeResponse::createNotJson(true, $m_flag, ['incon_to_id' => $model->timoperasi->to_ok_pl_id, 'incon_id' => $model->incon_id]);
      } else {
  ...
  {
    $title = 'Created ' . InformConcentTindakanDokter::title;
    //cek simpan draf/final
    //init model
    $model = new InformConcentTindakanDokter();
    //init model log 
    $modelLog = new Log();
    ...
    //cek allow crud data
    $chk_allow = HelperSpesial::checkAllowCRUDbyLayanan($model->incon_to_id);
    if (!$chk_allow->status) {
    ...
    if ($model->load(Yii::$app->request->post())) {
      ...
      $model->incon_tindakan_inform_consent = json_decode($model->incon_tindakan_inform_consent);
      if ($model->validate()) {
        if ($model->incon_final) {
          return MakeResponse::create(true, '', ['konfirm_final' => true]);
        }
        $save = $this->save($title, $modelLog, $model, []);
        if ($save->status) {
          return MakeResponse::create(true, $save->msg, ['konfirm_final' => false, 'id' => HelperGeneral::hashData($save->data['incon_to_id']), 'subid' => $save->data['incon_id']]);
        } else {
  ...
  {
    $title = 'Finalized ' . InformConcentTindakanDokter::title;
    //init model
    $model = new InformConcentTindakanDokter();
    //init model log 
    $modelLog = new Log();
    ...
    if ($model->load(Yii::$app->request->post())) {
      ...
      if ($model->validate()) {
        ...
        if ($save->status) {
          return MakeResponse::create(true, $save->msg, ['id' => HelperGeneral::hashData($save->data['incon_to_id']), 'subid' => $save->data['incon_id']]);
        } else {
          return MakeResponse::create(false, $save->msg);
        }
      } else {
  ...
  {
    $title = 'Updated ' . InformConcentTindakanDokter::title;
    //cek simpan draf/final/batalkan
    //init model order penunjang
    $model = $this->findModel($subid);
    // if($model->incon_dokter_pgw_id!=Akun::user()->id){
    ...
    if ($model->load(Yii::$app->request->post())) {
      ...
      $model->incon_tindakan_inform_consent = json_decode($model->incon_tindakan_inform_consent);
      if ($model->validate()) {
        if ($model->incon_final && !$model->incon_batal) {
          return MakeResponse::create(true, '', ['konfirm_final' => true, 'konfirm_batal' => false]);
        } else if ($model->incon_final && $model->incon_batal) {
  ...
  {
    $title = 'Canceled ' . InformConcentTindakanDokter::title;
    //init model
    $model = $this->findModel($subid);
    if (!$model->incon_final) {
  ...
  {
    $title = 'Deleted ' . InformConcentTindakanDokter::title;
    //init model
    $model = $this->findModel($subid);
    // if($model->incon_dokter_id!=Akun::user()->id){
    ...
    if ($model->validate()) {
      ...
      if ($save->status) {
        return MakeResponse::create(true, $save->msg, ['id' => HelperGeneral::hashData($save->data['incon_to_id'])]);
      } else {
        return MakeResponse::create(false, $save->msg);
      }
    } else {
  ...
  protected function initModelCreate($id, $layanan = array())
  {
    ...
    $timoperasidetail = TimOperasiDetail::find()->where(['tod_to_id' => $timoperasi->to_id, 'tod_jo_id' => 1])->one();
    $model->incon_to_id = $timoperasi->to_id;
    // $model->incon_dokter_pgw_id = $timoperasidetail->tod_pgw_id;
    // $model->incon_pemberi_informasi_pgw_id = $timoperasidetail->tod_pgw_id;
    $model->incon_keluarga_jenis_kelamin = 'Laki-Laki';
    $model->incon_setuju = '1';
    $model->incon_final = 1;
    $model->incon_batal = 0;
  ...
  {
    return InformConcentTindakanDokter::find()->where(['incon_to_id' => $to_id])->andWhere('incon_deleted_at is null')->orderBy(['incon_created_at' => SORT_DESC])->one();
  }
  protected function initModelUpdate($subid)
  {
    return InformConcentTindakanDokter::find()->where(['incon_id' => $subid])->andWhere('incon_deleted_at is null')->orderBy(['incon_created_at' => SORT_DESC])->one();
  }

  /**
   * Finds the InformConcentTindakanDokter model based on its primary key value.
   * If the model is not found, a 404 HTTP exception will be thrown.
   * @param integer $id
   * @return InformConcentTindakanDokter the loaded model
   * @throws NotFoundHttpException if the model cannot be found
   */
  protected function findModel($id)
  {
    if (($model = InformConcentTindakanDokter::findOne($id)) !== null) {
      return $model;
    }
