<?php

namespace app\controllers;

use Yii;
use app\models\bedahsentral\PraAnestesi;
use app\models\search\PraAnetesiSearch;
use yii\web\Controller;
use yii\web\NotFoundHttpException;
use yii\filters\VerbFilter;
use yii\helpers\Url;
use yii\helpers\ArrayHelper;
use app\components\HelperGeneral;
use app\components\Model;
use app\components\HelperSpesial;
use app\components\MakeResponse;
use app\components\mdcp\MdcpBase;
use app\components\mdcp\MdcpPraAnestesi;
use app\models\bedahsentral\Log;
use app\models\sdm\Suku;
use app\models\sdm\Agama;
use app\models\pendaftaran\Layanan;
use app\components\Akun;
use app\models\bedahsentral\TimOperasi;
use app\models\TimOperasiDetail;
use DateTime;

/**
 * PraAnetesiController implements the CRUD actions for PraAnetesi model.
 */
class PraAnestesiPasienController extends Controller
{
  /**
   * {@inheritdoc}
   */
  public function behaviors()
  {
    return [
      'verbs' => [
        'class' => VerbFilter::className(),
        'actions' => [
          'delete' => ['POST'],
        ],
      ],
    ];
  }

  /**
   * Lists all PraAnetesi models.
   * @return mixed
   */
  public function actionIndex($id)
  {
    $plid = HelperGeneral::convertLayananId($id);

    $timoperasi = TimOperasi::find()->where(['to_ok_pl_id' => $plid])->one();
    // echo "<pre>";print_r($plid);exit;
    $model = $this->initModelUpdateAuto($timoperasi->to_id);

    if ($model) {
      return $this->redirect(Url::to(['/pra-anestesi-pasien/update/', 'id' => $id, 'subid' => $model->ppa_id]));
    } else {
      return $this->redirect(Url::to(['/pra-anestesi-pasien/create/', 'id' => $timoperasi->to_ok_pl_id]));
    }
  }
  public function actionCreate($id)
  {
    $chk_pasien = HelperSpesial::getCheckPasien($id);
    if (!$chk_pasien->status) {
      // \Yii::$app->session->setFlash('warning', $chk_pasien->msg);
      return $this->redirect(Url::to(['/site/index/']));
    }
    $model = $this->initModelCreate($id, $chk_pasien->data);
    $title = PraAnestesi::ass_n;
    if (\Yii::$app->request->isAjax) {
      return $this->renderAjax('index', [
        'title' => $title,
        'model' => $model,
        'chk_pasien' => $chk_pasien->data
      ]);
    } else {
      $this->layout = 'main-pasien';
      return $this->render('index', [
        'title' => $title,
        'model' => $model,
        'chk_pasien' => $chk_pasien->data
      ]);
    }
  }
  //update & batalkan
  public function actionUpdate($id, $subid)
  {
    $model = $this->initModelUpdate($subid);
    if (!$model) {
      return $this->redirect(Url::to(['/site/index/']));
    }
    // Tampilkan dalam format tanggal
    $tanggal = date('Y-m-d', strtotime($model->ppa_tanggal_pukul));
    $jam = date('H:i', strtotime($model->ppa_tanggal_pukul));
    $model->ppa_tanggal_pukul = $tanggal . 'T' . $jam;
    // echo "<pre>";print_r($model);die;
    // $model->ppa_tanggal_pukul = '2018-06-12T19:30';

    $chk_pasien = HelperSpesial::getCheckPasien($id);
    if (!$chk_pasien->status) {
      // \Yii::$app->session->setFlash('warning', $chk_pasien->msg);
      return $this->redirect(Url::to(['/site/index/']));
    }
    $title = PraAnestesi::ass_n;
    if (\Yii::$app->request->isAjax) {
      return $this->renderAjax('index', [
        'title' => $title,
        'model' => $model,
        'chk_pasien' => $chk_pasien->data
      ]);
    } else {
      $this->layout = 'main-pasien';
      return $this->render('index', [
        'title' => $title,
        'model' => $model,
        'chk_pasien' => $chk_pasien->data
      ]);
    }
  }
  private function save($title, $modelLog, $model, $modelDetail, $final = false, $batal = false, $hapus = false)
  {
    $transaction = PraAnestesi::getDb()->beginTransaction();
    try {
      $s_flag = true;
      $m_flag = $title . ' Berhasil Disimpan';
      //save order
      if (!($s_flag = $model->save(false))) {
        $m_flag = $title . ' Gagal Disimpan..';
      }
      //save log
      if ($s_flag) {
        $modelLog->mlog_data_after = json_encode($model->attr());
        if (!($s_flag = $modelLog->save(false))) {
          $m_flag = 'Log ' . $title . ' Gagal Disimpan..';
        }
      }
      //save doc medis => karna save final
      // if ($final || $batal) {
      //   if ($s_flag) {
      //     $data = \Yii::$app->controller->renderPartial('doc', ['ppa_id' => $model->ppa_id]);
      //     $mdcp_base = MdcpBase::getLayanan($model->timoperasi->to_ok_pl_id);
      //     $mdcp = MdcpPraAnestesi::_set($model->ppa_id, $mdcp_base, [], $data, $batal);
      //     if (!($s_flag = $mdcp->status)) {
      //       $m_flag = $mdcp->msg;
      //     }
      //   }
      // }
      //cek finalisasi save
      if ($s_flag) {
        $transaction->commit();
        return MakeResponse::createNotJson(true, $m_flag, ['ppa_to_id' => $model->timoperasi->to_ok_pl_id, 'ppa_id' => $model->ppa_id]);
      } else {
        $transaction->rollBack();
        return MakeResponse::createNotJson(false, $m_flag);
      }
    } catch (\Exception $e) {
      $transaction->rollBack();
      return MakeResponse::createNotJson(false, 'Data Gagal Disimpan Karna : ' . $e);
      throw $e;
    } catch (\Throwable $e) {
      $transaction->rollBack();
      return MakeResponse::createNotJson(false, 'Data Gagal Disimpan Karna : ' . $e);
      throw $e;
    }
  }
  public function actionSaveInsert()
  {
    $title = 'Created ' . PraAnestesi::ass_n;
    //cek simpan draf/final
    //init model
    $model = new PraAnestesi();
    //init model log 
    $modelLog = new Log();
    $modelLog->mlog_type = Log::TYPE_CREATE;
    $modelLog->mlog_deskripsi = $title;
    $modelLog->mlog_data_before = json_encode($model->attr());
    //cek allow crud data
    $chk_allow = HelperSpesial::checkAllowCRUDbyLayanan($model->ppa_to_id);
    if (!$chk_allow->status) {
      return MakeResponse::create(false, $chk_allow->msg);
    }

    if ($model->load(Yii::$app->request->post())) {
      // echo'<pre/>';print_r($model);die();
      // $model->ppa_final = 0;
      // $model->ppa_fso_pernafasan = json_encode($model->ppa_fso_pernafasan);
      // $model->ppa_fso_kardiovaskuler = json_encode($model->ppa_fso_kardiovaskuler);
      // $model->ppa_fso_neuro = json_encode($model->ppa_fso_neuro);
      // $model->ppa_fso_endoktrin = json_encode($model->ppa_fso_endoktrin);
      // $model->ppa_fso_hepato = json_encode($model->ppa_fso_hepato);
      // $model->ppa_fso_lainnya = json_encode($model->ppa_fso_lainnya);
      if ($model->validate()) {
        // echo'<pre/>';print_r($model);die();
        if ($model->ppa_final) {
          return MakeResponse::create(true, '', ['konfirm_final' => true]);
        }
        $save = $this->save($title, $modelLog, $model, []);
        if ($save->status) {
          return MakeResponse::create(true, $save->msg, ['konfirm_final' => false, 'id' => HelperGeneral::hashData($save->data['ppa_to_id']), 'subid' => $save->data['ppa_id']]);
        } else {
          return MakeResponse::create(false, $save->msg);
        }
      } else {
        return MakeResponse::create(false, 'Data Gagal Disimpan', $model->errors);
      }
    }
  }
  public function actionSaveInsertFinal()
  {
    $title = 'Finalized ' . PraAnestesi::ass_n;
    //init model
    $model = new PraAnestesi();
    //init model log 
    $modelLog = new Log();
    $modelLog->mlog_type = Log::TYPE_CREATE;
    $modelLog->mlog_deskripsi = $title;
    $modelLog->mlog_data_before = json_encode($model->attr());
    //cek allow crud data
    $chk_allow = HelperSpesial::checkAllowCRUDbyLayanan($model->ppa_to_id);
    if (!$chk_allow->status) {
      return MakeResponse::create(false, $chk_allow->msg);
    }

    if ($model->load(Yii::$app->request->post())) {
      $model->setFinal();
      // $model->ppa_fso_pernafasan = json_encode($model->ppa_fso_pernafasan);
      // $model->ppa_fso_kardiovaskuler = json_encode($model->ppa_fso_kardiovaskuler);
      // $model->ppa_fso_neuro = json_encode($model->ppa_fso_neuro);
      // $model->ppa_fso_endoktrin = json_encode($model->ppa_fso_endoktrin);
      // $model->ppa_fso_hepato = json_encode($model->ppa_fso_hepato);
      // $model->ppa_fso_lainnya = json_encode($model->ppa_fso_lainnya);
      if ($model->validate()) {
        $save = $this->save($title, $modelLog, $model, [], true, false, false);
        if ($save->status) {
          return MakeResponse::create(true, $save->msg, ['id' => HelperGeneral::hashData($save->data['ppa_to_id']), 'subid' => $save->data['ppa_id']]);
        } else {
          return MakeResponse::create(false, $save->msg);
        }
      } else {
        return MakeResponse::create(false, 'Data Gagal Disimpan', $model->errors);
      }
    }
  }
  public function actionSaveUpdate($subid)
  {
    $title = 'Updated ' . PraAnestesi::ass_n;
    //cek simpan draf/final/batalkan
    //init model order penunjang
    $model = $this->findModel($subid);
    // if($model->ppa_dokter_pgw_id!=Akun::user()->id){
    //     return MakeResponse::create(false, 'Data Tidak Dapat Kelola');
    // }
    //init model log 
    $modelLog = new Log();
    $modelLog->mlog_type = Log::TYPE_UPDATE;
    $modelLog->mlog_deskripsi = $title;
    $modelLog->mlog_data_before = json_encode($model->attr());
    // echo'<pre/>';print_r($modelLog->mlog_data_before);die();

    //cek allow crud data
    $chk_allow = HelperSpesial::checkAllowCRUDbyLayanan($model->ppa_to_id);
    if (!$chk_allow->status) {
      return MakeResponse::create(false, $chk_allow->msg);
    }
    if ($model->load(Yii::$app->request->post())) {
      // $model->ppa_final = 0;
      // echo'<pre/>';print_r($model);die();
      // $model->ppa_fso_pernafasan = json_encode($model->ppa_fso_pernafasan);
      // $model->ppa_fso_kardiovaskuler = json_encode($model->ppa_fso_kardiovaskuler);
      // $model->ppa_fso_neuro = json_encode($model->ppa_fso_neuro);
      // $model->ppa_fso_endoktrin = json_encode($model->ppa_fso_endoktrin);
      // $model->ppa_fso_hepato = json_encode($model->ppa_fso_hepato);
      // $model->ppa_fso_lainnya = json_encode($model->ppa_fso_lainnya);
      if ($model->validate()) {
        if ($model->ppa_final && !$model->ppa_batal) {
          return MakeResponse::create(true, '', ['konfirm_final' => true, 'konfirm_batal' => false]);
        } else if ($model->ppa_final && $model->ppa_batal) {
          return MakeResponse::create(true, '', ['konfirm_final' => false, 'konfirm_batal' => true]);
        } else {
          $save = $this->save($title, $modelLog, $model, []);
          if ($save->status) {
            return MakeResponse::create(true, $save->msg, ['konfirm_final' => false, 'konfirm_batal' => false]);
          } else {
            return MakeResponse::create(false, $save->msg);
          }
        }
      } else {
        return MakeResponse::create(false, 'Data Gagal Disimpan', $model->errors);
      }
    }
  }
  public function actionSaveUpdateFinal($subid)
  {
    $title = 'Finalized ' . PraAnestesi::ass_n;
    //init model
    $model = $this->findModel($subid);
    if ($model->ppa_final) {
      return MakeResponse::create(false, 'Data Sudah Simpan Final');
    }
    //init model log 
    $modelLog = new Log();
    $modelLog->mlog_type = Log::TYPE_UPDATE;
    $modelLog->mlog_deskripsi = $title;
    $modelLog->mlog_data_before = json_encode($model->attr());

    //cek allow crud data
    $chk_allow = HelperSpesial::checkAllowCRUDbyLayanan($model->ppa_to_id);
    if (!$chk_allow->status) {
      return MakeResponse::create(false, $chk_allow->msg);
    }
    if ($model->load(Yii::$app->request->post())) {
      $model->setFinal();
      // $model->ppa_fso_pernafasan = json_encode($model->ppa_fso_pernafasan);
      // $model->ppa_fso_kardiovaskuler = json_encode($model->ppa_fso_kardiovaskuler);
      // $model->ppa_fso_neuro = json_encode($model->ppa_fso_neuro);
      // $model->ppa_fso_endoktrin = json_encode($model->ppa_fso_endoktrin);
      // $model->ppa_fso_hepato = json_encode($model->ppa_fso_hepato);
      // $model->ppa_fso_lainnya = json_encode($model->ppa_fso_lainnya);
      if ($model->validate()) {
        $save = $this->save($title, $modelLog, $model, [], true, false, false);
        if ($save->status) {
          return MakeResponse::create(true, $save->msg);
        } else {
          return MakeResponse::create(false, $save->msg);
        }
      } else {
        return MakeResponse::create(false, 'Data Gagal Disimpan', $model->errors);
      }
    }
  }
  public function actionSaveUpdateBatal($subid)
  {
    $title = 'Canceled ' . PraAnestesi::ass_n;
    //init model
    $model = $this->findModel($subid);
    if (!$model->ppa_final) {
      return MakeResponse::create(false, 'Data Tidak Dapat Dibatalkan');
    }
    //init model log 
    $modelLog = new Log();
    $modelLog->mlog_type = Log::TYPE_UPDATE;
    $modelLog->mlog_deskripsi = $title;
    $modelLog->mlog_data_before = json_encode($model->attr());
    //cek allow crud data
    $chk_allow = HelperSpesial::checkAllowCRUDbyLayanan($model->ppa_to_id);
    if (!$chk_allow->status) {
      return MakeResponse::create(false, $chk_allow->msg);
    }
    $model->setBatal();
    if ($model->validate()) {
      $save = $this->save($title, $modelLog, $model, [], false, true, false);
      if ($save->status) {
        return MakeResponse::create(true, $save->msg);
      } else {
        return MakeResponse::create(false, $save->msg);
      }
    } else {
      return MakeResponse::create(false, 'Data Gagal Disimpan', $model->errors);
    }
  }
  public function actionSaveUpdateHapus($id, $subid)
  {
    // echo "<pre>";print_r($subid);exit;
    $title = 'Deleted ' . PraAnestesi::ass_n;
    //init model
    $model = $this->findModel($subid);
    // if($model->ppa_dokter_pgw_id!=Akun::user()->id){
    //     return MakeResponse::create(false, 'Data Tidak Dapat Dihapus, Karna Anda Bukan Pembuat');
    // }
    if ($model->ppa_final) {
      return MakeResponse::create(false, 'Data Tidak Dapat Dihapus, Karna Sudah Simpan Final');
    }
    //init model log 
    $modelLog = new Log();
    $modelLog->mlog_type = Log::TYPE_UPDATE;
    $modelLog->mlog_deskripsi = $title;
    $modelLog->mlog_data_before = json_encode($model->attr());
    //cek allow crud data
    $chk_allow = HelperSpesial::checkAllowCRUDbyLayanan($model->ppa_to_id);
    if (!$chk_allow->status) {
      return MakeResponse::create(false, $chk_allow->msg);
    }
    $model->setDelete();
    if ($model->validate()) {
      $save = $this->save($title, $modelLog, $model, [], false, false, true);
      if ($save->status) {
        return MakeResponse::create(true, $save->msg, ['id' => HelperGeneral::hashData($save->data['ppa_to_id'])]);
      } else {
        return MakeResponse::create(false, $save->msg);
      }
    } else {
      return MakeResponse::create(false, 'Data Gagal Disimpan', $model->errors);
    }
  }
  protected function initModelCreate($id, $layanan = array(), $timoperasi = array())
  {
    $model = new PraAnestesi();
    if (!$layanan) {
      if (!is_numeric($id)) {
        $id = HelperGeneral::validateData($id);
      }
      $layanan = Layanan::find()->where(['id' => $id])->asArray()->one();
    }
    $timoperasi = TimOperasi::find()->where(['to_ok_pl_id' => $layanan['id']])->one();
    if (!empty($timoperasi)) {
      $model->ppa_diagnosis = $timoperasi->to_diagnosa_medis_pra_bedah;
      $model->ppa_rencana_tindakan = $timoperasi->to_tindakan_operasi;
      // $model->ppa_rencana_tindakan_elektif = $timoperasi->to_jenis_operasi_cito;
      $model->ppa_to_id = $timoperasi->to_id;

      $tanggal = date('Y-m-d', strtotime($timoperasi->to_tanggal_operasi));
      $jam = date('H:i', strtotime($timoperasi->to_tanggal_operasi));
      $model->ppa_tanggal_pukul = $tanggal . 'T' . $jam;
    }

    $model->ppa_final = 0;
    $model->ppa_batal = 0;
    return $model;
  }
  protected function initModelUpdateAuto($to_id)
  {
    return PraAnestesi::find()->where(['ppa_to_id' => $to_id])->andWhere('ppa_deleted_at is null')->orderBy(['ppa_created_at' => SORT_DESC])->one();
  }
  protected function initModelUpdate($subid)
  {
    return PraAnestesi::find()->where(['ppa_id' => $subid])->andWhere('ppa_deleted_at is null')->orderBy(['ppa_created_at' => SORT_DESC])->one();
  }

  /**
   * Finds the PraAnestesi model based on its primary key value.
   * If the model is not found, a 404 HTTP exception will be thrown.
   * @param integer $id
   * @return PraAnestesi the loaded model
   * @throws NotFoundHttpException if the model cannot be found
   */
  protected function findModel($id)
  {
    if (($model = PraAnestesi::findOne($id)) !== null) {
      return $model;
    }

    throw new NotFoundHttpException('The requested page does not exist.');
  }
}
