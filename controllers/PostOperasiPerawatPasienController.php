...
use app\models\search\PostOperasiPerawatSearch;
...
use app\components\mdcp\MdcpPostOperasiPerawat;
use app\models\bedahsentral\Log;
use app\models\pendaftaran\Layanan;
use app\components\Akun;
use app\controllers\AsesmenAwalPerawatController;
use app\models\bedahsentral\TimOperasi;
use app\models\bedahsentral\TimOperasiDetail;
use app\models\bedahsentral\IntraOperasiPerawat;

/**
 * MedisPostOperasiPerawatController implements the CRUD actions for PostOperasiPerawat model.
 */
class PostOperasiPerawatPasienController extends Controller
{
  ...
  public function actionIndex($id)
  {
    ...
    if ($model) {
      return $this->redirect(Url::to(['/post-operasi-perawat-pasien/update/', 'id' => $id, 'subid' => $model->psop_id]));
    } else {
      return $this->redirect(Url::to(['/post-operasi-perawat-pasien/create/', 'id' => $timoperasi->to_ok_pl_id]));
    }
  }
  public function actionCreate($id)
  {
    ...
    $title = PostOperasiPerawat::ass_psop; //
  ...
  public function actionUpdate($id, $subid)
  {
    ...
    $model->psop_barang_diserahkan_via_prwt_rgn = json_decode($model->psop_barang_diserahkan_via_prwt_rgn);
  ...
  {
    $transaction = PostOperasiPerawat::getDb()->beginTransaction();
    try {
      $s_flag = true;
      ...
      // if ($final || $batal) {
      //   if ($s_flag) {
      //     $data = \Yii::$app->controller->renderPartial('doc', ['psop_id' => $model->psop_id]);
      //     $mdcp_base = MdcpBase::getLayanan($model->timoperasi->to_ok_pl_id);
      //     $mdcp = MdcpPostOperasiPerawat::_set($model->psop_id, $mdcp_base, [], $data, $batal);
      ...
      if ($s_flag) {
        $transaction->commit();
        return MakeResponse::createNotJson(true, $m_flag, ['psop_to_id' => $model->timoperasi->to_ok_pl_id, 'psop_id' => $model->psop_id]);
      } else {
  ...
  {
    $title = 'Created ' . PostOperasiPerawat::ass_psop;
    //cek simpan draf/final
    //init model
    $model = new PostOperasiPerawat();
    //init model log 
    $modelLog = new Log();
    ...
    //cek allow crud data
    $chk_allow = HelperSpesial::checkAllowCRUDbyLayanan($model->psop_to_id);
    if (!$chk_allow->status) {
    ...
    if ($model->load(Yii::$app->request->post())) {
      // $model->psop_final = 0;
      $model->psop_barang_diserahkan_via_prwt_rgn = json_encode($model->psop_barang_diserahkan_via_prwt_rgn);
      if ($model->validate()) {
        ...
        if ($save->status) {
          return MakeResponse::create(true, $save->msg, ['konfirm_final' => false, 'id' => HelperGeneral::hashData($save->data['psop_to_id']), 'subid' => $save->data['psop_id']]);
        } else {
  ...
  {
    $title = 'Finalized ' . PostOperasiPerawat::ass_psop;
    //init model
    $model = new PostOperasiPerawat();
    //init model log 
    $modelLog = new Log();
    ...
    if ($model->load(Yii::$app->request->post())) {
      ...
      if ($model->validate()) {
        ...
        if ($save->status) {
          return MakeResponse::create(true, $save->msg, ['id' => HelperGeneral::hashData($save->data['psop_to_id']), 'subid' => $save->data['psop_id']]);
        } else {
          return MakeResponse::create(false, $save->msg);
        }
      } else {
  ...
  public function actionSaveUpdate($subid)
  {
    ...
    if ($model->load(Yii::$app->request->post())) {
      ...
      $model->psop_barang_diserahkan_via_prwt_rgn = json_encode($model->psop_barang_diserahkan_via_prwt_rgn);
      if ($model->validate()) {
        if ($model->psop_final && !$model->psop_batal) {
          return MakeResponse::create(true, '', ['konfirm_final' => true, 'konfirm_batal' => false]);
        } else if ($model->psop_final && $model->psop_batal) {
  ...
  {
    $title = 'Canceled ' . PostOperasiPerawat::ass_psop;
    //init model
    $model = $this->findModel($subid);
    if (!$model->psop_final) {
  ...
  {
    $title = 'Deleted ' . PostOperasiPerawat::ass_psop;
    //init model
    $model = $this->findModel($subid);
    // if($model->psop_perawat_id!=Akun::user()->id){
    ...
    if ($model->validate()) {
      ...
      if ($save->status) {
        return MakeResponse::create(true, $save->msg, ['id' => HelperGeneral::hashData($save->data['psop_to_id'])]);
      } else {
        return MakeResponse::create(false, $save->msg);
      }
    } else {
  ...
  protected function initModelCreate($id, $layanan = array())
  {
    ...
    $intra = IntraOperasiPerawat::find()->where(['iop_to_id' => $timoperasi->to_id])->one();
    if ($intra) {
      $model->psop_masalah = $intra->iop_masalah;
      $model->psop_tindakan = $intra->iop_tindakan;
      $model->psop_implementasi = $intra->iop_implementasi;
      $model->psop_evaluasi = $intra->iop_evaluasi;
    }
    $model->psop_to_id = $timoperasi->to_id;
    $model->psop_final = 0;
    $model->psop_batal = 0;

    // //set anemnesa
    // $model->psop_ruang_pemulihan = 'Ya';
    // $model->psop_keadaan_umum = 'Baik';
    // $model->psop_tingkat_kesadaran = 'Apatis';
    // $model->psop_pernapasan = 'Spontan';
    // $model->psop_sirkulasi = 'kebiruan';
    // $model->psop_turgor_kulit = 'Elastis';
    // $model->psop_posisi_klien = 'Terlentang';
    // $model->psop_pasang_drain = 'Tidak';
    // $model->psop_jaringan_pa_form = 'Tidak';
    // $model->psop_resep = 'Tidak';
    // $model->psop_integritas_kulit = 'Utuh';
    // $model->psop_tulang = 'Tidak';
    return $model;
  }
  protected function initModelUpdateAuto($to_id)
  {
    return PostOperasiPerawat::find()->where(['psop_to_id' => $to_id])->andWhere('psop_deleted_at is null')->orderBy(['psop_created_at' => SORT_DESC])->one();
  }
  protected function initModelUpdate($subid)
  {
    return PostOperasiPerawat::find()->where(['psop_id' => $subid])->andWhere('psop_deleted_at is null')->orderBy(['psop_created_at' => SORT_DESC])->one();
  }
  protected function findModel($id)
  {
    if (($model = PostOperasiPerawat::findOne($id)) !== null) {
      return $model;
    }
