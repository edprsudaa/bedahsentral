...
use app\components\mdcp\MdcpPreOperasiPerawatOk;
...
class PreOperasiPerawatOkPasienController extends \yii\web\Controller
{
  ...
  /**
   * Lists all PreOperasiPerawatOk models.
   * @return mixed
   */
  public function actionIndex($id)
  {
    ...
    // echo "<pre>";print_r($timoperasi);die;
    $model = $this->initModelUpdateAuto($timoperasi->to_id);

    if ($model) {
      return $this->redirect(Url::to(['/pre-operasi-perawat-ok-pasien/update/', 'id' => $id, 'subid' => $model->pop_id]));
    } else {
      return $this->redirect(Url::to(['/pre-operasi-perawat-ok-pasien/create/', 'id' => $timoperasi->to_ok_pl_id]));
    }
  }
  public function actionCreate($id)
  {
    ...
    $title = PreOperasiPerawatOk::ass_pop;
  ...
  public function actionUpdate($id, $subid)
  {
    ...
    // $agama = ArrayHelper::map(DmAgama::find()->all(), 'agama', 'agama');
    // $suku = ArrayHelper::map(DmSuku::find()->all(), 'nama', 'nama');
    $title = PreOperasiPerawatOk::ass_pop;
    if (\Yii::$app->request->isAjax) {
      return $this->renderAjax('index', [
        'title' => $title,
  ...
  {
    $transaction = PreOperasiPerawatOk::getDb()->beginTransaction();
    try {
      $s_flag = true;
      ...
      //     $mdcp = MdcpPreOperasiPerawatOk::_set($model->pop_id, $mdcp_base, [], $data, $batal);
  ...
  {
    $title = 'Created ' . PreOperasiPerawatOk::ass_pop;
    //cek simpan draf/final
    //init model
    $model = new PreOperasiPerawatOk();
    //init model log 
    $modelLog = new Log();
  ...
  {
    $title = 'Finalized ' . PreOperasiPerawatOk::ass_pop;
    //init model
    $model = new PreOperasiPerawatOk();
    //init model log 
    $modelLog = new Log();
  ...
  public function actionSaveUpdate($subid)
  {
    ...
    if ($model->load(Yii::$app->request->post())) {
      $model->pop_pgw_id_ok = Akun::user()->id;
      // $model->pop_final_ok = 0;
      if ($model->validate()) {
        if ($model->pop_final_ok && !$model->pop_batal_ok) {
          return MakeResponse::create(true, '', ['konfirm_final' => true, 'konfirm_batal' => false]);
        } else if ($model->pop_final_ok && $model->pop_batal_ok) {
  ...
  {
    $title = 'Canceled ' . PreOperasiPerawatOk::ass_pop;
    //init model
    $model = $this->findModel($subid);
    if (!$model->pop_final_ok) {
  ...
  {
    $title = 'Deleted ' . PreOperasiPerawatOk::ass_pop;
    //init model
    $model = $this->findModel($subid);
    // if($model->pop_pgw_id_ok!=Akun::user()->id){
  ...
  {
    $model =  PreOperasiPerawatOk::find()
  ...
  {
    return PreOperasiPerawatOk::find()
      ->where(['pop_id' => $subid, 'pop_deleted_at' => null])
      ->orderBy(['pop_created_at' => SORT_DESC])->one();
  }

  /**
   * Finds the AsesmenAwalPerawat model based on its primary key value.
   * If the model is not found, a 404 HTTP exception will be thrown.
   * @param integer $id
   * @return AsesmenAwalPerawat the loaded model
   * @throws NotFoundHttpException if the model cannot be found
   */
  protected function findModel($id)
  {
    if (($model = PreOperasiPerawatOk::findOne($id)) !== null) {
      return $model;
    }
