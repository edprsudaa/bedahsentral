$("#ttvintra").on("submit", function (e) {
  e.preventDefault();
  e.stopImmediatePropagation();
  $.ajax({
    url: baseUrl + "/ttv-intra-anestesi/create",
    type: "POST",
    contentType: false,
    processData: false,
...
$("#medikasi").on("submit", function (e) {
  ...
  e.stopImmediatePropagation();
  $.ajax({
    url: baseUrl + "/medikasi-intra-anestesi/create",
    type: "POST",
    contentType: false,
    processData: false,
    data: new FormData(this),
...
$("#form-cairan-masuk").on("submit", function (e) {
  ...
  e.stopImmediatePropagation();
  $.ajax({
    url: baseUrl + "/cairan-masuk-intra-anestesi/create",
    type: "POST",
    contentType: false,
    processData: false,
    data: new FormData(this),
...
$("#form-cairan-keluar").on("submit", function (e) {
  ...
  e.stopImmediatePropagation();
  $.ajax({
    url: baseUrl + "/cairan-keluar-intra-anestesi/create",
    type: "POST",
    contentType: false,
    processData: false,
    data: new FormData(this),
...
$(document).ready(function () {
  $("#mia")
    .on("beforeSubmit", function (e) {
      // var url_base64 = document
      //   .getElementById("myChart")
      //   .toDataURL("image/png");
      // $("#intra-base64").val(url_base64);
      var btn = $(".btn-submit");
      fbtn.setLoading(btn, "proses...");
      var post_data = $(this).serialize();
      var subid = btn.attr("data-subid");
      var url_update_draf =
        baseUrl + "/intra-anestesi-pasien/save-update?subid=" + subid;
      var url_update_final =
        baseUrl + "/intra-anestesi-pasien/save-update-final?subid=" + subid;
      var url_update_batal =
        baseUrl + "/intra-anestesi-pasien/save-update-batal?subid=" + subid;
      $.ajax({
        url: url_update_draf,
        type: "post",
        dataType: "json",
            ...
            success: function (data) {
              if (data.status) {
                ...
                window.location.href =
                  baseUrl + "/intra-anestesi-pasien/index?id=" + data.data.id;
              } else {
                fmsg.w(data.msg);
              }
            },
