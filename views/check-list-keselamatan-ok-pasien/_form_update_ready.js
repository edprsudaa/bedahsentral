$(".btn-cetak").click(function () {
  let id = $(this).data("id");
  var w = window.open(
    "/cetak/cetak-checklist-keselamatan?checklist_id=" + id,
    "",
...
$(document).ready(function () {
  $("#pcok")
    .on("beforeSubmit", function (e) {
      ...
      var url_update_draf =
        baseUrl +
        "/check-list-keselamatan-ok-pasien/save-update?subid=" +
        subid;
      var url_update_final =
        baseUrl +
        "/check-list-keselamatan-ok-pasien/save-update-final?subid=" +
        subid;
      var url_update_batal =
        baseUrl +
        "/check-list-keselamatan-ok-pasien/save-update-batal?subid=" +
        subid;
      $.ajax({
        url: url_update_draf,
        type: "post",
            ...
            success: function (data) {
              if (data.status) {
                ...
                window.location.href =
                  baseUrl +
                  "/check-list-keselamatan-ok-pasien/index?id=" +
                  data.data.id;
              } else {
                fmsg.w(data.msg);
              }
