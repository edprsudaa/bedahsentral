$(".btn-cetak").click(function () {
  let id = $(this).data("id");
  var w = window.open(
    "/cetak/cetak-jumlah-kasa?pjki_id=" + id,
    "",
...
$(document).ready(function () {
  $("#pjki")
    .on("beforeSubmit", function (e) {
      ...
      var url_update_draf =
        baseUrl +
        "/penggunaan-jumlah-kasa-dan-instrumen-pasien/save-update?subid=" +
        subid;
      var url_update_final =
        baseUrl +
        "/penggunaan-jumlah-kasa-dan-instrumen-pasien/save-update-final?subid=" +
        subid;
      var url_update_batal =
        baseUrl +
        "/penggunaan-jumlah-kasa-dan-instrumen-pasien/save-update-batal?subid=" +
        subid;
      $.ajax({
        url: url_update_draf,
        type: "post",
            ...
            success: function (data) {
              if (data.status) {
                ...
                window.location.href =
                  baseUrl +
                  "/penggunaan-jumlah-kasa-dan-instrumen-pasien/index?id=" +
                  data.data.id;
              } else {
                fmsg.w(data.msg);
              }
