    ...
    <table border="0" class="table table-sm table-form">
      ...
      <tr>
        <td colspan="6">
          ...
          <?php
          echo $form->field($model, 'lap_op_jenis_operasi', ['options' => ['class' => 'form-group custom-margin']])->inline(true)->radioList(['1' => 'Cyto', '0' => 'Elektif'])->label(false);
          ?>
        </td>
      </tr>
      <tr>
        <td colspan="6">
          ...
          $lap_op_jaringan_operasi_dikirim = HelperGeneral::getValueCustomRadio($lap_op_jaringan_operasi_dikirim, $model->lap_op_jaringan_operasi_dikirim);
          ?>
        </td>
      </tr>
      <tr>
        ...
        <div class="btn-group-vertical">
          ...
          if (!$model->lap_op_batal) {
            if (!$model->lap_op_final) {
              ...
              echo $form->field($model, 'lap_op_batal')->widget(SwitchInput::classname(), [
                'pluginOptions' => [
                  'size' => 'mini',
                  'onText' => 'Batal',
                  'offText' => 'Non-batal',
            ...
            echo Html::submitButton('<i class="fas fa-save"></i> Simpan', ['class' => 'btn btn-success btn-submit', 'data-subid' => $model->lap_op_id]);
            echo Html::button('<i class="fas fa-sync"></i> Segarkan', ['class' => 'btn btn-warning btn-segarkan']);
            if (!$model->lap_op_final) {
              echo Html::button('<i class="fas fa-trash-alt"></i> Hapus', ['class' => 'btn btn-danger btn-hapus-draf', 'data-url' => Url::to(['/laporan-operasi-pasien/save-update-hapus', 'id' => Yii::$app->request->get('id'), 'subid' => $model->lap_op_id])]);
            }
            echo Html::a('<i class="fas fa-plus-circle"></i> Tambah Baru', ['/laporan-operasi-pasien/create', 'id' => Yii::$app->request->get('id')], ['class' => 'btn btn-primary']);
            //PRINT
            if ($model->lap_op_final) {
          ...
          // echo Html::button((($model->lap_op_batal) ? 'NB:Doc.Batal' : (($model->lap_op_final) ? 'NB:Doc.Final' : 'NB:Doc.Draf')), ['class' => 'btn btn-default']);
          ?>
        </div>
      </div>
    </div>
