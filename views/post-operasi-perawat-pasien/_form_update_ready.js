...
ya_ruang_pemulihan = (we) => {
  $("#masukrr").show();
};

tidak_ruang_pemulihan = (we) => {
  $("#masukrr").hide();
};
...
$(".btn-cetak").click(function () {
  let id = $(this).data("id");
  var w = window.open(
    "/cetak/cetak-post-operasi?psop_id=" + id,
    "",
...
$("#psop_ruang_pemulihan_it").on("input change focus paste", function (e) {
  $("#psop_ruang_pemulihan").val($(this).val());
  $("#psop_ruang_pemulihan").prop("checked", true);
});
$("#psop_pasang_drain_it").on("input change focus paste", function (e) {
  $("#psop_pasang_drain").val($(this).val());
  $("#psop_pasang_drain").prop("checked", true);
});
$("#psop_jaringan_pa_form_it").on("input change focus paste", function (e) {
  $("#psop_jaringan_pa_form").val($(this).val());
  $("#psop_jaringan_pa_form").prop("checked", true);
});

$(document).ready(function () {
  var timerAjaxIcd = 0;
  var currentRequesttAjaxIcd = null; //utk cancel req pedding karna ada req new search icd
  $("postoperasiperawat-psop_masalah").atwho({
    searchKey: "name",
    at: "@",
    limit: 100,
  ...
  $("postoperasiperawat-psop_tindakan").atwho({
    searchKey: "name",
    at: "@",
    limit: 100,
    displayTpl: "<li data-value='${key}'>${name}</li>",
  ...
  $("#psop")
    .on("beforeSubmit", function (e) {
      ...
      var url_update_draf =
        baseUrl + "/post-operasi-perawat-pasien/save-update?subid=" + subid;
      var url_update_final =
        baseUrl +
        "/post-operasi-perawat-pasien/save-update-final?subid=" +
        subid;
      var url_update_batal =
        baseUrl +
        "/post-operasi-perawat-pasien/save-update-batal?subid=" +
        subid;
      $.ajax({
        url: url_update_draf,
        type: "post",
            ...
            success: function (data) {
              if (data.status) {
                ...
                window.location.href =
                  baseUrl +
                  "/post-operasi-perawat-pasien/index?id=" +
                  data.data.id;
              } else {
                fmsg.w(data.msg);
              }
