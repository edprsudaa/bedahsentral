    ...
    <div class="col-lg-11">
      <table class="table table-sm table-form">
            ...
            <div style="display: flex;justify-content:space-around;">
              ...
              echo $form->field($model, 'psop_ruang_pemulihan', ['options' => ['class' => 'form-group custom-margin']])->radioList($psop_ruang_pemulihan, [
                'item' => function ($index, $label, $name, $checked, $value) {
                  ...
                  <label onClick = "ya_ruang_pemulihan(this.id)" for="customRadio1" class="custom-control-label"> ' . $label . '</label></div>';
                }
              ])->label(false);
              $psop_ruang_pemulihan = HelperGeneral::getValueCustomRadio($psop_ruang_pemulihan, $model->psop_ruang_pemulihan);
              ?>

              <div id="masukrr" style="<?= (!$psop_ruang_pemulihan['v'] ? 'visibility: visible' : 'display: none') ?>">
                <div>
        ...
        <tr>
          ...
          <td><label>:</label></td>
          <td><?= $form->field($model, 'psop_bedrest', ['options' => ['class' => 'form-group custom-margin']])->textInput(['placeholder' => 'Bedrest ....... jam', 'class' => 'form-control form-control-sm'])->label(false); ?></td>
          <td><?= $form->field($model, 'psop_head_up', ['options' => ['class' => 'form-group custom-margin']])->textInput(['placeholder' => 'Head Up ....... °', 'class' => 'form-control form-control-sm'])->label(false); ?></td>
        </tr>
        <tr>
          ...
          <div class="btn-group-vertical">
            ...
            if (!$model->psop_batal) {
              if (!$model->psop_final) {
                ...
                echo $form->field($model, 'psop_batal')->widget(SwitchInput::classname(), [
                  'pluginOptions' => [
                    'size' => 'mini',
                    'onText' => 'Batal',
                    'offText' => 'Non-batal',
              ...
              echo Html::submitButton(Yii::t('app', '{title}', ['title' => '<i class="fas fa-save"></i> Simpan']), ['class' => 'btn btn-success btn-submit', 'data-subid' => $model->psop_id]);
              echo Html::button(Yii::t('app', '{title}', ['title' => '<i class="fas fa-sync"></i> Segarkan']), ['class' => 'btn btn-warning btn-segarkan']);
              if (!$model->psop_final) {
                echo Html::button(Yii::t('app', '{title}', ['title' => '<i class="fas fa-trash-alt"></i> Hapus']), ['class' => 'btn btn-danger btn-hapus-draf', 'data-url' => Url::to(['/post-operasi-perawat-pasien/save-update-hapus', 'id' => Yii::$app->request->get('id'), 'subid' => $model->psop_id])]);
              }
              echo Html::a(Yii::t('app', '{title}', ['title' => '<i class="fas fa-plus-circle"></i> Tambah Baru']), ['/post-operasi-perawat-pasien/create', 'id' => Yii::$app->request->get('id')], ['class' => 'btn btn-primary']);
              if ($model->psop_final) {
            ...
            // echo Html::button(Yii::t('app', '{title}', ['title' => (($model->psop_batal) ? 'NB:Doc.Batal' : (($model->psop_final) ? 'NB:Doc.Final' : 'NB:Doc.Draf'))]), ['class' => 'btn btn-default']);
            ?>
          </div>
        </div>
      </div>
