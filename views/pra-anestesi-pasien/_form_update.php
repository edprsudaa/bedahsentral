  ...
  <div class="col-lg-11">
    <table class="table table-sm table-form">
      <tr>
        ...
        <td style="width: 15%;"><label>Diagnosis</label></td>
        <td><label>:</label></td>
        <td style="width: 35%;">
    ...
    <table class="table table-sm">
      ...
      <tr>
        ...
        <td colspan="3">
          ...
          <div id="atas" style="margin-bottom:0;<?= (($ppa_gigi_normal['v']) ? 'visibility:visible' : 'display:none') ?>">
            ...
            <div class="custom-control custom-radio custom-control-inline palsuatas" style="<?= (($ppa_gigi_normal['v'] == 'Atas Semua') || ($ppa_gigi_normal['v'] == 'Atas Sebagian') ? 'visibility:visible' : 'display:none') ?>">
          ...
          <div id="bawah" style="margin-bottom:1rem;<?= (($ppa_gigi_normal['v']) ? 'visibility:visible' : 'display:none') ?>">
            ...
            <div class="custom-control custom-radio custom-control-inline palsubawah" style="<?= (($ppa_gigi_normal['v'] == 'Bawah Semua') || ($ppa_gigi_normal['v'] == 'Bawah Sebagian') ? 'visibility:visible' : 'display:none') ?>">
      ...
      <tr>
        ...
        <td colspan="3">
          ...
          ?>
          <table align="center" id="tabelclass" style="<?= (($model->ppa_jalan_nafas == "Ada masalah, Sebutkan") ? "visibility:visible" : "display:none") ?>">
            <tr>
              <th>class 1</th>
              <th>class 2</th>
              <th>class 3</th>
        ...
        <div class="btn-group-vertical">
          ...
          if (!$model->ppa_batal) {
            if (!$model->ppa_final) {
              ...
              echo $form->field($model, 'ppa_batal')->widget(SwitchInput::classname(), [
                'pluginOptions' => [
                  'size' => 'mini',
                  'onText' => 'Batal',
                  'offText' => 'Non-batal',
            ...
            echo Html::submitButton(Yii::t('app', '{title}', ['title' => '<i class="fas fa-save"></i> Simpan']), ['class' => 'btn btn-success btn-submit', 'data-subid' => $model->ppa_id]);
            echo Html::button(Yii::t('app', '{title}', ['title' => '<i class="fas fa-sync"></i> Segarkan']), ['class' => 'btn btn-warning btn-segarkan']);
            if (!$model->ppa_final) {
              echo Html::button(Yii::t('app', '{title}', ['title' => '<i class="fas fa-trash-alt"></i> Hapus']), ['class' => 'btn btn-danger btn-hapus-draf', 'data-url' => Url::to(['/pra-anestesi-pasien/save-update-hapus', 'id' => Yii::$app->request->get('id'), 'subid' => $model->ppa_id])]);
            }
            echo Html::a(Yii::t('app', '{title}', ['title' => '<i class="fas fa-plus-circle"></i> Tambah Baru']), ['/pra-anestesi-pasien/create', 'id' => Yii::$app->request->get('id')], ['class' => 'btn btn-primary']);
          } else {
            echo Html::button(Yii::t('app', '{title}', ['title' => '<i class="fas fa-sync"></i> Segarkan']), ['class' => 'btn btn-warning btn-segarkan']);
            echo Html::a(Yii::t('app', '{title}', ['title' => '<i class="fas fa-plus-circle"></i> Tambah Baru']), ['/pra-anestesi-pasien/create', 'id' => Yii::$app->request->get('id')], ['class' => 'btn btn-primary']);
          }
          // echo Html::button(Yii::t('app', '{title}', ['title' => (($model->ppa_batal) ? 'NB:Doc.Batal' : (($model->ppa_final) ? 'NB:Doc.Final' : 'NB:Doc.Draf'))]), ['class' => 'btn btn-default']);
          ?>
        </div>
      </div>
    </div>
