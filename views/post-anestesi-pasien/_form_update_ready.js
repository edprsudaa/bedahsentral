$("#form-ttv-post").on("submit", function (e) {
  e.preventDefault();
  e.stopImmediatePropagation();
  $.ajax({
    url: baseUrl + "/ttv-post-anestesi/create",
    type: "POST",
    contentType: false,
    processData: false,
    ...
    dataType: "JSON",
    beforeSend: function () {
      $(".btn-post").html("Menyimpan...");
    },
    success: function (res) {
      // console.log(res.desc);
      $("#form-ttv-post")[0].reset();
      $(".btn-post").html("Tambah");
      fmsg.s(res.desc);
      // grafikpost();
    },
    error: function (err) {
...
$(document).ready(function () {
  $("#mpa")
    .on("beforeSubmit", function (e) {
      ...
      var url_update_draf =
        baseUrl + "/post-anestesi-pasien/save-update?subid=" + subid;
      var url_update_final =
        baseUrl + "/post-anestesi-pasien/save-update-final?subid=" + subid;
      var url_update_batal =
        baseUrl + "/post-anestesi-pasien/save-update-batal?subid=" + subid;
      $.ajax({
        url: url_update_draf,
        type: "post",
        dataType: "json",
            ...
            success: function (data) {
              if (data.status) {
                ...
                window.location.href =
                  baseUrl + "/post-anestesi-pasien/index?id=" + data.data.id;
              } else {
                fmsg.w(data.msg);
              }
            },
