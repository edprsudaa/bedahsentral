...
$(document).ready(function () {
  ...
  $("#preoperasiperawatok-pop_masalah").atwho({
    searchKey: "name",
    at: "@",
    limit: 100,
  ...
  $("#preoperasiperawatok-pop_tindakan").atwho({
    searchKey: "name",
    at: "@",
    limit: 100,
    displayTpl: "<li data-value='${key}'>${name}</li>",
  ...
  $("#pre")
    .on("beforeSubmit", function (e) {
      ...
      var url_update_draf =
        baseUrl + "/pre-operasi-perawat-ok-pasien/save-update?subid=" + subid;
      var url_update_final =
        baseUrl +
        "/pre-operasi-perawat-ok-pasien/save-update-final?subid=" +
        subid;
      var url_update_batal =
        baseUrl +
        "/pre-operasi-perawat-ok-pasien/save-update-batal?subid=" +
        subid;
      $.ajax({
        url: url_update_draf,
        type: "post",
            ...
            success: function (data) {
              if (data.status) {
                ...
                window.location.href =
                  baseUrl +
                  "/pre-operasi-perawat-ok-pasien/index?id=" +
                  data.data.id;
              } else {
                fmsg.w(data.msg);
              }
