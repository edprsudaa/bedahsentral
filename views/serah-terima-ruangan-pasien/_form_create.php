...
<?= $form->field($model, 'mstr_batal')->hiddenInput()->label(false); ?>
    ...
    <table width="100%" style="border:1px solid;" border="1">
      <tbody>
        <tr>
          ...
          <td style="width:15%;"><label><?= $model->getAttributeLabel('mstr_tgl_masuk_ruangan') ?></label></td>
          <td>:</td>
          <td>
    ...
    <div class="row icon-sticky">
      <div class="col-lg-12">
        <div class="btn-group-vertical">
          <?= $form->field($model, 'mstr_final')->widget(SwitchInput::classname(), [
            'pluginOptions' => [
          ...
          <?= Html::a(Yii::t('app', '{title}', ['title' => '<i class="fas fa-times"></i> Kembali']), ['/serah-terima-ruangan-pasien/index', 'id' => Yii::$app->request->get('id')], ['class' => 'btn btn-danger']) ?>
        </div>
      </div>
    </div>
  </div>
