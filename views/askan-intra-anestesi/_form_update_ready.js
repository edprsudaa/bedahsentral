$(document).ready(function () {
  $("#aia")
    .on("beforeSubmit", function (e) {
      ...
      // $("#post-base64").val(url_base64);
      var btn = $(".btn-submit");
      fbtn.setLoading(btn, "proses...");
      var post_data = $(this).serialize();
      var subid = btn.attr("data-subid");
      var url_update_draf =
        baseUrl + "/askan-intra-anestesi/save-update?subid=" + subid;
      var url_update_final =
        baseUrl + "/askan-intra-anestesi/save-update-final?subid=" + subid;
      var url_update_batal =
        baseUrl + "/askan-intra-anestesi/save-update-batal?subid=" + subid;
      $.ajax({
        url: url_update_draf,
        type: "post",
        dataType: "json",
            ...
            success: function (data) {
              if (data.status) {
                ...
                window.location.href =
                  baseUrl + "/askan-intra-anestesi/index?id=" + data.data.id;
              } else {
                fmsg.w(data.msg);
              }
            },
  ...
  $("#aia_obat_inhalasi_it").on("input change focus paste", function (e) {
    $("#aia_obat_inhalasi").val($(this).val());
    $("#aia_obat_inhalasi").prop("checked", true);
  });
  $("#aia_posisi_operasi_it").on("input change focus paste", function (e) {
    $("#aia_posisi_operasi").val($(this).val());
    $("#aia_posisi_operasi").prop("checked", true);
  });
});
