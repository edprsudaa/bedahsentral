...
<div class="medis-intra-operasi-perawat-form">
  ...
  <?= $form->field($model, 'iop_batal')->hiddenInput()->label(false); ?>
  <div class="row">
    <div class="col-lg-11">
      ...
      <table width="100%" border="1" class="table table-sm table-form">
        ...
        <tr>
          ...
          <td>
            <?php
            $iop_kateter_urin = ['Tidak' => 'Tidak', 'Ya' => 'Ya', 'Dalam OK' => 'Dalam OK', 'Ruangan' => 'Ruangan'];

            echo $form->field($model, 'iop_kateter_urin', ['options' => ['class' => 'form-group custom-margin']])->radioList($iop_kateter_urin, [
              'item' => function ($index, $label, $name, $checked, $value) {
                return '<div class="custom-control custom-radio custom-control-inline" id="radio' . $index . '">
                  <input onClick = "kateterurin(this.id)" class="custom-control-input" type="radio" id="customRadio' . $index . '" name="' . $name . '" value="' . $value . '" ' . ($checked ? "checked" : "") . '>
                <label onClick = "kateterurin()" for="customRadio' . $index . '" class="custom-control-label"> ' . $label . '</label>
                </div>';
              }
            ])->label(false);
          ...
          <td colspan="3" id="ku_dipasang_oleh">
      ...
      <div class="row icon-sticky">
        <div class="col-lg-12">
          <div class="btn-group-vertical">
            ...
            <?= Html::a(Yii::t('app', '{title}', ['title' => '<i class="fas fa-times"></i> Kembali']), ['/intra-operasi-perawat-pasien/index', 'id' => Yii::$app->request->get('id')], ['class' => 'btn btn-danger']) ?>
          </div>
        </div>
      </div>
    </div>
