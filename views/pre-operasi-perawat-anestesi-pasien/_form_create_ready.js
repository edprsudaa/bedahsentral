$(document).ready(function () {
  ...
  $("#preoperasiperawatanestesi-pop_masalah").atwho({
    searchKey: "name",
    at: "@",
    limit: 100,
  ...
  $("#preoperasiperawatanestesi-pop_tindakan").atwho({
    searchKey: "name",
    at: "@",
    limit: 100,
    displayTpl: "<li data-value='${key}'>${name}</li>",
  ...
  // FORM CUSTOM RADIO
  $("#pop_tingkat_kesadaran_it").on("input change focus paste", function (e) {
    $("#pop_tingkat_kesadaran").val($(this).val());
    $("#pop_tingkat_kesadaran").prop("checked", true);
  });
  $("#pop_pernapasan_it").on("input change focus paste", function (e) {
    $("#pop_pernapasan").val($(this).val());
    $("#pop_pernapasan").prop("checked", true);
  });
  ...
  $("#pre")
    .on("beforeSubmit", function (e) {
      ...
      var url_insert_draf =
        baseUrl + "/pre-operasi-perawat-anestesi-pasien/save-insert";
      var url_insert_final =
        baseUrl + "/pre-operasi-perawat-anestesi-pasien/save-insert-final";
      $.ajax({
        url: url_insert_draf,
        type: "post",
        dataType: "json",
                      ...
                      success: function (data) {
                        if (data.status) {
                          ...
                          window.location.href =
                            baseUrl +
                            "/pre-operasi-perawat-anestesi-pasien?id=" +
                            data.data.id +
                            "&subid" +
                            data.data.subid;
                        } else {
  ...
  // $("#total-gcs").click(function () {
  //   var e = $("#gcs-e").val();
  //   var m = $("#gcs-m").val();
  //   var v = $("#gcs-v").val();
  //   var totalgcs = Number(e) + Number(m) + Number(v);
