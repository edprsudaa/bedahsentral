...
<button type="button" class="btn btn-info" data-toggle="modal" data-target="#modal-default">Lihat Preview Lokasi</button>
<br><br>
<div class="row">
  <div class="col-lg-11">
    <div class="row">
        ...
        <table width="100%">
          ...
          <tr>
            <td height="610px" width="230px">
              ...
              <?= $form->field($model, 'mlo_catatan', ['options' => ['class' => 'form-group custom-margin']])->hiddenInput(['class' => 'form-control form-control-sm', 'placeholder' => '.....', 'rows' => 25, 'id' => 'bckgrnd'])->label(false); ?>
        ...
        <div class="btn-group-vertical">
          ...
          if (!$model->mlo_batal) {
            if (!$model->mlo_final) {
              ...
              echo $form->field($model, 'mlo_batal')->widget(SwitchInput::classname(), [
                'pluginOptions' => [
                  'size' => 'mini',
                  'onText' => 'Batal',
                  'offText' => 'Non-batal',
            ...
            echo Html::submitButton(Yii::t('app', '{title}', ['title' => '<i class="fas fa-save"></i> Simpan']), ['class' => 'btn btn-success btn-submit', 'data-subid' => $model->mlo_id]);
            echo Html::button(Yii::t('app', '{title}', ['title' => '<i class="fas fa-sync"></i> Segarkan']), ['class' => 'btn btn-warning btn-segarkan']);
            if (!$model->mlo_final) {
              echo Html::button(Yii::t('app', '{title}', ['title' => '<i class="fas fa-trash-alt"></i> Hapus']), ['class' => 'btn btn-danger btn-hapus-draf', 'data-url' => Url::to(['/lokasi-operasi-pasien/save-update-hapus', 'id' => Yii::$app->request->get('id'), 'subid' => $model->mlo_id])]);
            }
            echo Html::a(Yii::t('app', '{title}', ['title' => '<i class="fas fa-plus-circle"></i> Tambah Baru']), ['/lokasi-operasi-pasien/create', 'id' => Yii::$app->request->get('id')], ['class' => 'btn btn-primary']);
          } else {
            echo Html::button(Yii::t('app', '{title}', ['title' => '<i class="fas fa-sync"></i> Segarkan']), ['class' => 'btn btn-warning btn-segarkan']);
            echo Html::a(Yii::t('app', '{title}', ['title' => '<i class="fas fa-plus-circle"></i> Tambah Baru']), ['/lokasi-operasi-pasien/create', 'id' => Yii::$app->request->get('id')], ['class' => 'btn btn-primary']);
          }
          // echo Html::button(Yii::t('app', '{title}', ['title' => (($model->mlo_batal) ? 'NB:Doc.Batal' : (($model->mlo_final) ? 'NB:Doc.Final' : 'NB:Doc.Draf'))]), ['class' => 'btn btn-default']);
          ?>
        </div>
      </div>
    </div>
...
<div class="modal fade prev" id="modal-default">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body" id="testing">
        <img src="<?= $model->mlo_gambar_marking ?>" alt="image" class="preview" id="prev" />
      </div>
