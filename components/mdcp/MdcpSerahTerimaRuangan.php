...
use app\models\medis\SerahTerimaRuangan;
use app\models\medis\Log;
use yii\helpers\ArrayHelper;
class MdcpSerahTerimaRuangan
{
    const kode = 'DM_0000105';
    const data_type= DocClinicalPasien::data_type_html_base64;
    public static function _get()
    {
        //$modelMdcp=DocClinicalPasien::findOne($model->mstr_mdcp_id);
        //DECRYPT DATA
    ...
    public static function _set($id,$layanan=array(),$registrasi=array(),$data,$batal=false){
        ...
        $model=SerahTerimaRuangan::find()->where(['mstr_id'=>$id])->one();
        if($model->mstr_mdcp_id){
            //1.update mdcp
            $modelMdcp=DocClinicalPasien::findOne($model->mstr_mdcp_id);
            ...
            if($modelMdcp->save(false)){
                ...
                $model->mstr_mdcp_id=$modelMdcp->mdcp_id;
                if($model->save(false)){
                    //3.create log
                    $modelLog=new Log();
                    $modelLog->scenario="mdcp";
