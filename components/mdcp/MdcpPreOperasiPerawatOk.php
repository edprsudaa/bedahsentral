...
class MdcpPreOperasiPerawatOk
{
  const kode = 'DM_0000108';
  const data_type = DocClinicalPasien::data_type_html_base64;
  public static function _get()
  {
    // $modelMdcp=DocClinicalPasien::findOne($model->pop_mdcp_id_ok);
    // //DECRYPT DATA
  ...
  public static function _set($id, $layanan = array(), $registrasi = array(), $data, $batal = false)
  {
    ...
    $model = PreOperasiPerawatOk::find()->where(['pop_id' => $id])->one();
    if ($model->pop_mdcp_id_ok) {
      //1.update mdcp
      $modelMdcp = DocClinicalPasien::findOne($model->pop_mdcp_id_ok);
      ...
      if ($modelMdcp->save(false)) {
        ...
        $model->pop_mdcp_id_ok = $modelMdcp->id_doc_clinical_pasien;
        if ($model->save(false)) {
          //3.create log
          $modelLog = new Log();
          $modelLog->scenario = "mdcp";
