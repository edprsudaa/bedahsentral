...
class MdcpLaporanOperasi
{
  const kode = 'DM_0000104';
  const data_type = DocClinicalPasien::data_type_html_base64;
  public static function _get()
  {
    //$modelMdcp=DocClinicalPasien::findOne($model->lap_op_mdcp_id);
    //DECRYPT DATA
  ...
  public static function _set($id, $layanan = array(), $registrasi = array(), $data, $batal = false)
  {
    ...
    $model = LaporanOperasi::find()->where(['lap_op_id' => $id])->one();
    if ($model->lap_op_mdcp_id) {
      //1.update mdcp
      $modelMdcp = DocClinicalPasien::findOne($model->lap_op_mdcp_id);
      ...
      if ($modelMdcp->save(false)) {
        ...
        $model->lap_op_mdcp_id = $modelMdcp->id_doc_clinical_pasien;
        if ($model->save(false)) {
          //3.create log
          $modelLog = new Log();
          $modelLog->scenario = "mdcp";
