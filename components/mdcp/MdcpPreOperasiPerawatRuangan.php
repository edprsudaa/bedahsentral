...
class MdcpPreOperasiPerawatRuangan
{
  const kode = 'DM_0000100';
  const data_type = DocClinicalPasien::data_type_html_base64;
  public static function _get()
  {
    // $modelMdcp=DocClinicalPasien::findOne($model->pop_mdcp_id_ruangan);
    // //DECRYPT DATA
  ...
  public static function _set($id, $layanan = array(), $registrasi = array(), $data, $batal = false)
  {
    ...
    $model = PreOperasiPerawatRuangan::find()->where(['pop_id' => $id])->one();
    if ($model->pop_mdcp_id_ruangan) {
      //1.update mdcp
      $modelMdcp = DocClinicalPasien::findOne($model->pop_mdcp_id_ruangan);
      ...
      if ($modelMdcp->save(false)) {
        ...
        $model->pop_mdcp_id_ruangan = $modelMdcp->id_doc_clinical_pasien;
        if ($model->save(false)) {
          //3.create log
          $modelLog = new Log();
          $modelLog->scenario = "mdcp";
