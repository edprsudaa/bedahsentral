...
var fbtn={
    setLoading:function(btn,btn_txt=''){
        if(btn_txt){
            $(btn).html('<i class="fas fa-spinner fa-spin"></i>'+btn_txt).attr('disabled',true);
        }else{
            $(btn).html('<i class="fas fa-spinner fa-spin"></i> Proses...').attr('disabled',true);
        }
    },
...
function iframeModal(url,content_id,id) {
    ...
    $(content_id+"-content").css('height','500px').css('html','500px').html(iframe);
